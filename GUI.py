...
class GUI():
    def __init__(self):
        ...
        self.keywords_description = Label(
            self.gui, text="Keywords bitte nur eins pro Zeile (mit Enter trennen)")
        ...
        self.start_crawler = Button(
            self.gui, bd=1, width=80, height=5, text="Start", command=self.start)
    ...
    def getDriver(self):
        ...
        if(OPEN_BROWSER):
            ...
            driver = webdriver.Chrome(
                executable_path=DRIVER_PATH, options=options, chrome_options=options)
            return driver
        else:
            options = webdriver.ChromeOptions()
    ...
    def start(self):
        ...
        if self.keywords.compare("end-1c", "==", "1.0"):
            ...
            for keyword in keywords.readlines():
                   ...
                   try:
                        results[keyword] = crawler.read_ads(
                            keyword, self.getDriver())
